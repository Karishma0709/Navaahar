  ...
  return (
    <nav className="shadow-md fixed w-full top-0 left-0 z-50 bg-white">
      <div className="container mx-auto px-4 md:px-8 lg:px-12 flex justify-between items-center">
        ...
        <button
          className="text-green-600 md:hidden text-3xl p-2"
      ...
      {/* ✅ Mobile & Tablet Menu (With Close Button Inside) */}
      <motion.div
        initial={{ x: '100%' }}
        animate={{ x: isOpen ? '0%' : '100%' }}
        transition={{ duration: 0.3 }}
        className={`fixed top-0 right-0 h-screen w-3/4 sm:w-2/4 bg-green-700 shadow-lg flex flex-col items-center py-10 space-y-6 transform transition-transform ${
      ...
      >
        ...
        {['Home', 'About', 'Product', 'Gallery', 'Career'].map((item) => (
          <Link
            key={item}
            to={`/${item.toLowerCase()}`}
